// BLOG: http://blog.yoursway.com/2009/07/3-painful-ways-to-obtain-stack-trace-in.html
// STATIC: http://crashkitapp.appspot.com/static/javascript/crashkit-javascript.js
// GIT: http://github.com/andreyvit/crashkit-api 
// GIST: http://gist.github.com/156138
...
// FUTURE: http://ajaxian.com/archives/exceptionhub#comments (wrap functions in try/catch)
// FUTURE: http://eriwen.com/javascript/stacktrace-update/
...
CrashKit.report = (function()
{
    ...
    {
        // MIKE: if exception is a string then fake exception error
        if (typeof (ex) == 'string')
        {
            // NOTE: exceptions only seem to be strings when they come from java
            try
            {
                ex = new Error(ex, 'JAVA', -1);
            } catch (ex2) { }
        }
    ...
    // add error handler to a window
    var addErrorHandler = function(win)
    {
        // check if error handler is already assigned
        if (typeof win == 'object' && 
            typeof win.onerror != 'function')
        {
            // add error handler
            win.onerror = onerrorHandler;
        }
    };

    // listen for default window error handler
    addErrorHandler(window);

    report.subscribe = subscribe;
    report.addErrorHandler = addErrorHandler;
...
CrashKit.computeStackTrace = (function() {
    ...
    var computeStackTrace = function(ex, depth) {
        ...
        return { 'mode': 'failed', 'name': ex.name, 'message': ex.message };
    };
