...
EU.Generic = function () {
    this.greedyEvaluation = function (varsWB, varsFunc) {
        ...
        for (var i = 0; i < varCount.length; i++) {
            if (varCount[i].bindable && varCount[i].count > iMax) iMax = varCount[i].count;
        }
        // return the list with the max count of bindable variables
        r = [];
        for (var i = 0; i < varsFunc.length; i++) {
            if ((varCount[i].bindable && varCount[i].count === iMax) || varCount[i].count === 0) r.push(varsFunc[i].name);
        }
        return r;
    };
    ...
    this.fullEvaluation = function (varsWB, varsFunc) {
        ...
        for (var i = 0; i < varsFunc.length; i++) {
            ...
            for (var j = 0; j < v.length; j++) {
                if ((SirVariableManager.isVarBindable(v[j])) && (varsWB.indexOf(v[j]) === -1)) {
                    fBind = false;
                    break;
                }
            }
            if (fBind) func.push(varsFunc[i]);
        }
        return func;
    };
    ...
    this.countVarBindable = function (varsWB, varFunc) {
        ...
        for (var i = 0; i < v.length; i++) {
            if (SirVariableManager.isVarBindable(v[i])) {
    ...
    this.convertElementNamesToVarNames = function (vNames) {
        ...
        for (var j = 0; j < vNames.length; j++) {
            var v = SirVariableManager.getVariableByObjectName(vNames[j]);
            if (v) {
                r.push(v.getName());
            } else {
                EU.debug('Cannot find a varible name for element:' + vNames[j]);
                r.push(vNames[j]); // save under original name
            }
        }
        return r;
    };
    ...
    this.getConstantValues = function () {
        ...
        r = { vNames: [], vValues: [] };
        var c = SirVariableManager.getConstants();
    ...
    this.getImplicitValues = function (varFuncs) {
        ...
        var impvarsDeclaredNames = SirVariableManager.getVariableNames(['implicit']); // get list of all implicit vars declared in xml
        for (var i = 0; i < varFuncs.length; i++) {
            var v = varFuncs[i].vars;
            for (var j = 0; j < v.length; j++) {
                if (impvarsDeclaredNames.indexOf(v[i]) >= 0) {  // found an implicit var
                    if (imvarsRequiredNames.indexOf(v[i]) === -1) {  // if not in the list already
                        imvarsRequiredNames.push(v[i]);              // save
                        var sirVar = SirVariableManager.getVariableByName(v[i]); // get var object
                        imvarsRequiredValues.push(sirVar.getValues());  // get all values and save
                    }
                }
            }
        }
    ...
    this.getFuncVars = function () {
        vFuncs = [];
        var fn = SirFunctionManager.getFunctions();
    ...
    this.packVariablesforWhiteboard = function (v) {
        ...
        for (var i = 0; i < v.length; i++) {
            ...
            var sirVar = SirVariableManager.getVariableByName(v[i].name);
