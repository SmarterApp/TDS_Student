...
function onglobalkeyevent(evt, type, key, keyCode, charCode)
{
    ...
    {
        if (!isKeyNavigational()) stopEvent(); // stop key if not safe
        return;
    }
...
var KeyManager =
{
  ...
  /*
  attachFrame: function(frameName)
  {
    var frame;
    
    if ( typeof(frameName) == 'undefined')
    {
        // attach to first frame if no frame name was defined
    ...
    if (frame == null) 
    {
        Util.log('keyboard: Could not attach to the frame ' + frameName);
    }
